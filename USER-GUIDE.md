...
5. Back in the app, **Choose the ledger file** (or paste Claude's table) → **Add … and check them**. Bring in more ledger files whenever you like — items already in the ledger are skipped. An item is recognised by what it says (its date, activity, details, annexure and source reference) or by a **Ledger ID** if the file gives one, never by which sheet or row it sat on, so two ledger files may both have a sheet called "Evidence ledger" without being taken for each other, and a corrected version of a row lands on the item it corrects.
6. Before anything is added, the page says what it found: how many are new, how many are **already in the ledger**, how many are **outside the assessment period**, and any **Duplicate?** cards — two or more rows of that one file describing the same work on the same date under the same annexure, shown together so you keep whichever says most instead of counting the hours twice. Untick or re-tick any of them.
7. Each item then gets a verdict — **Not recorded** (add it as a row), **Recorded, not fully**, **More than the source shows**, **On another date?**, **Recorded** (use it as the row's proof). **Dismiss** anything irrelevant. A notice calling all HoDs to a meeting while you were head is marked **HoD then — presumed present**; adding it records the meeting with that note.
8. Work dated **outside the assessment period** is kept but scores nothing, so it is filed behind one line — *3 items outside the period, not counted* — with **Show them** when you want to look. Nothing is deleted.

Nothing is ever added to an existing row across a date that doesn't match. Where a ledger item and the row it is linked to carry different dates, the card reads **On another date?** and says which two dates disagree, rather than quietly merging one into the other.
